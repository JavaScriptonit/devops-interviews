...
## 1. Нужно развернуть инстансы MongoDB в Google Cluster. У нас есть 2 НП. 1 преэмтипал пулл и 2ой кор пулл. Задача: поды MongoDB шедулились только на определенном ноде пулле (кор пулл). Как технически решить эту задачу? (k8s)
...
## 3. Как жестко прикрепить под к ноде? Можно ли сделать это через ноде селектор?
