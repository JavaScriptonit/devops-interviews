...
# Вопросы из записи в iCloud - Виртуальный ассистент (Тех собес Сбер devices 1/2):
...
## 13. Чем лучше оркестрация k8s в сравнении с systemd units и ansible, заменит ли оркестрацию? (k8s)
...
## 14. Написать код на bash, который создаст 10 файлов по заданному пути с названиями: 1a, 2a и тд (Bash)

`/Users/andreyshabunov/PhpstormProjects/devops-interviews/sber/devices/task` - скрипты для создания и удаления файлов


## 15. Можно ли заменить кавычки "" другим символом? (Bash)
```
for ((i=1; i<=10; i++))
do
  filename="${path}/${i}a"
...
# Вопросы из записи в iCloud - Виртуальный ассистент (Тех собес Сбер devices 2/2):

## 1. Какие CI использовал? (CI/CD)
...
## 2. Какую CI выберешь в собственной компании и почему? (CI/CD)
...
## 3. Как заменить drod-down menu Jenkins и GitHub в GitLab для параметризованной сборки? (CI/CD)
...
## 7. Terraform apply (main.tf) стал выполняться 30мин. Что делать? (Terraform)
...
## 8. Как поделить файл на модули? Разделить main.tf на несколько файлов module1.tf, module2.tf и module3.tf и вынести туда части из main.tf ? (Terraform)
...
## 9. Какие могут быть избысточные настройки в main.tf ? (Terraform)
...
## 10. Как быть если в main.tf создаётся много объектов и они все нужны? Создаётся 1 базовая сеть и 1 подсеть и 100 машин в этой сети. (Terraform)
...
## 10.1. Зачем использовать Terragrunt и пояснение к п.10 (Terraform):
...
## 12. Что такое DRY подход? (Terraform)
...
## 13. С какими облаками работал? (Провайдеры)
...
## 13. В чем разница между ними? (Провайдеры)
